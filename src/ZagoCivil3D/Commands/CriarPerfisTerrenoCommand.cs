...
/// <summary>
/// Comando que cria surface profiles (perfis do terreno natural e terraplenagem)
/// para todos os alinhamentos do desenho, replicando a rotina Dynamo 1.1.
/// Usa janela modeless para nao bloquear o Civil 3D.
/// </summary>
public class CriarPerfisTerrenoCommand
{
    private static CriarPerfisTerrenoWindow? m_janelaAtiva;

    [CommandMethod("ZAGO_CRIAR_PERFIS_TERRENO", CommandFlags.Session)]
    public void Executar()
    {
        ...
        try
        {
            editor.WriteMessage("\n[ZagoCivil3D] Iniciando criacao de perfis do terreno.");
            ...
            var janela = new CriarPerfisTerrenoWindow(superficies, estilosPerfil, conjuntosRotulos, camadas);
    ...
    private static void AoConfirmarJanela(object? remetente, CriarPerfisTerrenoRequest request)
    {
        Document? documento = AplicacaoAutoCad.DocumentManager.MdiActiveDocument;
        if (documento == null)
            return;
        ...
        try
        {
            ...
            CriarPerfisTerrenoResultado resultado =
                PerfilTerrenoService.Executar(documentoCivil, banco, editor, request);

            editor.WriteMessage("\n");
            editor.WriteMessage("\n===== RESUMO PERFIS DO TERRENO =====");
        ...
        {
            editor.WriteMessage($"\n[ZagoCivil3D] Erro no comando de perfis do terreno: {ex.Message}");
